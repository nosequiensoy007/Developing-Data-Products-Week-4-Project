    ...
    navbarPage(
        ...
        tabPanel(
            ...
            sidebarPanel(
                ...
                sliderInput("cyl", "Cilindros", min = minCylinders, max = maxCylinders, value = defaultCar$cyl, step = 1),
                bsTooltip(id = "cyl", title = "Numbero de cilindros", placement = "right", options = list(container = "body")),
                
                sliderInput("disp", "Desplazamiento", min = minDisp, max = maxDisp, value = defaultCar$disp, step = 1),
                bsTooltip(id = "disp", title = "Engine displacement (in cu.in.)", placement = "right", options = list(container = "body")),
                
                sliderInput("hp", "Poder", min = minHp, max = maxHp, value = defaultCar$hp, step = 1),
                bsTooltip(id = "hp", title = "Engine's gross horsepower in HP", placement = "right", options = list(container = "body")),
                
                sliderInput("drat", "Relacion vs Eje Trasero", min = minDrat, max = maxDrat, value = defaultCar$drat, step = 0.01),
                bsTooltip(id = "drat", title = "Rear axle ratio", placement = "right", options = list(container = "body")),
                
                sliderInput("wt", "Peso", min = minWt, max = maxWt, value = defaultCar$wt, step = 1),
                bsTooltip(id = "wt", title = "Weight in 1000 lbs", placement = "right", options = list(container = "body")),
                
                sliderInput("qsec", "Tiempo 1/4 mile", min = minQsec, max = maxQsec, value = defaultCar$qsec, step = 0.1),
                bsTooltip(id = "qsec", title = "1/4 mile time in seconds", placement = "right", options = list(container = "body")),
                
                radioButtons("vs", label = "Tipo de Motor", choices = list("Forma V" = 0, "Recto" = 1), selected = 1, inline = TRUE),
                bsTooltip(id = "vs", title = "Engine cylinder configuration (V-shape or straight)", placement = "right", options = list(container = "body")),
                
                radioButtons("am", label = "Transmision", choices = list("Automatico" = 0, "Manual" = 1), selected = 1, inline = TRUE),
                bsTooltip(id = "am", title = "Transmission type (automatic or manual)", placement = "right", options = list(container = "body")),
                
                sliderInput("gear", "Engranaje", min = minGear, max = maxGear, value = defaultCar$gear, step = 1),
                bsTooltip(id = "gear", title = "Number of forward gears in the transmission", placement = "right", options = list(container = "body")),
                
                sliderInput("carb", "Carburador", min = minCarb, max = maxCarb, value = defaultCar$carb, step = 1),
            ...
            mainPanel(
                ...
                h3("Prediccion Millas Por Galon"),
                
                br(),
                
                p("Tu consumo sera de: "),
